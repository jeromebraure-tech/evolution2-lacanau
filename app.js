...
async function checkAccess(showMessage=true){
  const result=await liveAccess();if(result.access?.status==='ACTIF'){$('instructor').value=result.access.name;$('instructor').readOnly=true;renderActivities();renderConditionalForm()}render();
