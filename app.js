...
const colors={Kite:'#ff6b35',Wingfoil:'#118ab2',Paddle:'#06a77d','Ski nautique':'#7b61ff',Efoil:'#00a6a6',Pumpfoil:'#4361ee','Fat bike':'#8a5a44',Trottinette:'#9b5de5',Groupe:'#ef476f'};
...
function render(){
  ...
  else{$('accessName').value='';$('accessEmail').value='';$('accessStatus').textContent=''}
...
async function checkAccess(showMessage=true){
  ...
  try{const result=await jsonp({action:'status',token:access.token}),updated={...access,status:result.status||'INCONNU',name:result.name||access.name};setAccess(updated);if(updated.status==='ACTIF'){$('instructor').value=updated.name;$('instructor').readOnly=true}render();if(showMessage)toast(updated.status==='ACTIF'?'Accès autorisé':'Accès non encore autorisé');return updated.status==='ACTIF'}catch(error){if(showMessage)toast('Impossible de vérifier l’accès');return false}
}
